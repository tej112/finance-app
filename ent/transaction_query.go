...
type TransactionQuery struct {
	...
	withTransfer *BankTransferQuery
...
func (tq *TransactionQuery) QueryBank() *BankQuery {
	query := (&BankClient{config: tq.config}).Query()
...
func (tq *TransactionQuery) QueryTransfer() *BankTransferQuery {
	query := (&BankTransferClient{config: tq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (tq *TransactionQuery) Clone() *TransactionQuery {
	...
	return &TransactionQuery{
		...
		withBank:     tq.withBank.Clone(),
		withCategory: tq.withCategory.Clone(),
		withTransfer: tq.withTransfer.Clone(),
...
func (tq *TransactionQuery) WithBank(opts ...func(*BankQuery)) *TransactionQuery {
	query := (&BankClient{config: tq.config}).Query()
...
func (tq *TransactionQuery) WithTransfer(opts ...func(*BankTransferQuery)) *TransactionQuery {
	query := (&BankTransferClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withTransfer = query
	return tq
}
...
func (tq *TransactionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Transaction, error) {
	...
	if query := tq.withBank; query != nil {
		if err := tq.loadBank(ctx, query, nodes, nil,
			func(n *Transaction, e *Bank) { n.Edges.Bank = e }); err != nil {
			return nil, err
		}
	}
	...
	if query := tq.withTransfer; query != nil {
		if err := tq.loadTransfer(ctx, query, nodes, nil,
			func(n *Transaction, e *Bank_Transfer) { n.Edges.Transfer = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TransactionQuery) loadBank(ctx context.Context, query *BankQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Bank)) error {
...
func (tq *TransactionQuery) loadTransfer(ctx context.Context, query *BankTransferQuery, nodes []*Transaction, init func(*Transaction), assign func(*Transaction, *Bank_Transfer)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*Transaction)
	for i := range nodes {
	...
	query.Where(bank_transfer.IDIn(ids...))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	}
...
func (tq *TransactionQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := tq.ctx.Fields; len(fields) > 0 {
		...
		if tq.withBank != nil {
			_spec.Node.AddColumnOnce(transaction.FieldBankID)
		}
		if tq.withCategory != nil {
			_spec.Node.AddColumnOnce(transaction.FieldCategoryID)
		}
		if tq.withTransfer != nil {
			_spec.Node.AddColumnOnce(transaction.FieldTransferID)
		}
	}
